...
public class ArgumentsParser {
    public void parse(String[] args) {
        try {
            ...
            name.append(args[args.length - 1]);
            ...
            switch (args[1]) {
                ...
                case "-expenses": {
                    parliament.build(Integer.parseInt(args[0]));
                    System.out.println(parliament.getSumOfExpenses(name.toString()));
                    break;
                }
                case "-small": {
                    parliament.build(Integer.parseInt(args[0]));
                    System.out.println(parliament.getSmallExpenses(name.toString()));
                    break;
                }
                case "-tripscount": {
                    if (args.length != 2) throw new IllegalArgumentException("Too many arguments");
